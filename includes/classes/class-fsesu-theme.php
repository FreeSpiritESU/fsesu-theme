...
class FSESU_Theme {
    ...
    private $allowed_tags = "<p><a><em><strong><figcaption><figure><div><video><embed><audio><cite><code><del><i><q><strike><li><ul><ol>";
    private $excerpt_length = 120;
    private $excerpt_more = ' &hellip; ';
    private $number_random_images = 5;
    ...
    public function __construct() {
        ...
        // Add auto embed support to excerpts
        add_filter( 'the_excerpt', array( $GLOBALS['wp_embed'], 'autoembed' ), 9 );
        
        // Allow some html tags in the excerpt
        ...
        add_action( 'random_images', array( $this, 'random_images' ), 10, 1 );
        add_action( 'gallery_images', array( $this, 'gallery_images' ), 10, 1 );
    ...
    public function theme_support() {
        ...
        set_post_thumbnail_size( 110, 110, false );
        add_theme_support( 'menus' );
        add_theme_support( 'editor-style' );
        add_theme_support( 'automatic-feed-links' );
        add_theme_support( 'post-formats', array( 
            'gallery', 'image', 'status', 'video' ) );
    ...
    public function menus() {
        ...
        register_nav_menus(
            array(
                'main-menu' => __( 'Main Menu', 'fsesu' ),
                'footer-menu' => __( 'Footer Menu', 'fsesu' )
            )
        );
    }
    ...
    public function scripts() {
        ...
        wp_register_script( 'fsesu-functions', FSESU_SCRIPTS . '/functions.js', array( 'jquery' ), false, true );
        wp_enqueue_script( 'fsesu-functions' );
    ...
    public function styles() {
        ...
        if ( ! is_admin() ) {
            ...
            wp_register_style( 'normalize', FSESU_STYLES . '/vendor/normalize.css', null, '3.0.1' );
    ...
    public function trim_excerpt( $text ) {
        ...
        if ( '' == $text ) {
            ...
            if ( count( $words ) > $excerpt_length ) {
                array_pop( $words );
                array_push( $words, $this->excerpt_more );
                $text = implode( ' ', $words );
            }
        }
        return $text;
                ...
                <nav class="<?php echo $nav_id; ?>">
                    ...
                    <div class="nav-previous alignleft">
                      <?php ( $type == 'post' )  
                        ...
                        : next_posts_link('<i class="fa fa-chevron-left"></i> Older posts'); ?>   
                    </div>
                    <div class="nav-next alignright"> 
                      <?php ( $type == 'post' )  
                        ? next_post_link( '%link', '%title <i class="fa fa-chevron-right"></i>' )  
                        : previous_posts_link('Newer posts <i class="fa fa-chevron-right"></i>'); ?>  
                    </div> 
                </nav>
<?php }
    ...
    public function random_images( $number ) {
        
        $ids = '';
        $counter = 0;
        $number = ( $number ) ? $number : $this->number_random_images;
        $featured = get_post_thumbnail_id();
        $attachments = $this->get_attachments( $number, 'rand', $featured );
        ...
        echo do_shortcode('[gallery include="' . $ids . '" link="file"]'); 
    }
    ...
    public function gallery_images() {
        ...
        $output = __( 'Gallery contains ', 'fsesu' );
        $output .= sprintf( _n( '1 photo', '%s photos', $count, 'fsesu' ), $count );
    ...
    private function get_attachments( $number = -1, $orderby = 'post_date', $exclude = '' ) {
        global $post;
